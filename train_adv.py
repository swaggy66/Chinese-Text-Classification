...
if __name__ == "__main__":
    ...
    for epoch in range(opt.num_epochs):
        ...
        for i, batch in bar:
            ...
            loss_adv = loss_f(adv_outputs, labels)
            acc_adv = accuracy(adv_outputs, labels, topk=[1])[0]
            loss_adv.backward()
            fgm.restore()

            optimizer.step()

            logger.add_scalar('loss', loss_adv.item(), total_step)
            logger.add_scalar('acc', acc_adv.item(), total_step)
